...
echo BV
	...
	Echo to !FilePath!
	Echo.
	REM Powershell -Command "Invoke-WebRequest '!ScriptUrl!' -OutFile '!FilePath!'"
	If "%UpdateArg%"=="no" (
		...
		Echo to !FilePath!
		Echo.
		REM Powershell -Command "Invoke-WebRequest '!ServiceUrl!' -OutFile '!ServiceFilePath!'"
		If "%BatDownload%"=="yes" (
			...
			Echo to !FilePath!
			Echo.
			REM Powershell -Command "Invoke-WebRequest '!BatUrl!' -OutFile '!BatFilePath!'"
		)
	)
	Set DownloadBV=no
	goto CheckRun
...
:BVWebVer
	...
	Set WebBaseVerBV=0
	Set WebSubVerBV=0
	REM Powershell -Command "Invoke-WebRequest '%URL_Base%BlackViperScript/master/Version/Version.csv' -OutFile '%Temp%\BV-Ver.csv'"
...
:W10
	...
	If %BatDownload%==yes (
		...
		Echo to !FilePath!
		Echo.
		REM Powershell -Command "Invoke-WebRequest '!BatUrl!' -OutFile '!FilePath!'"
	)
...
:W10WebVer
	...
	Set WebBaseVerW10=0
	Set WebSubVerW10=0
	REM Powershell -Command "Invoke-WebRequest '%URL_Base%Win10Script/master/Version/Version.csv' -OutFile '%Temp%\W10-Ver.csv'"
...
:CheckRun
	...
	echo !FilePath!
	If "%UpdateArg%"=="yes" (
		REM Powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& '!FilePath!' !MiscArg!" -Verb RunAs
		echo Got to Run Script with Args
		REM Exit
	)
	If "%RunArg%"=="yes" (
		If "%DownloadBV-W10%"=="done" (
			...
			REM Powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& '!FilePath!'" -Verb RunAs
			echo Got to Run Script
		)
		echo 2
		REM Exit
	)
